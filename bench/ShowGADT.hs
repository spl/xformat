...
-- | Heterogeneous list of format descriptors intended for 'showsf''.
data FL :: (* -> *) -> * where
  End  :: Format f => !f -> FL (S f)
  (:.) :: (Format f, Functor g) => !f -> !(FL g) -> FL (S f :.: g)
infixr 8 :.

-- | The fold on FL
showsFL :: Functor f => FL f -> f ShowS
showsFL (End fmt)   = showsf' fmt
showsFL (fmt :. fs) = showsf' fmt <> showsFL fs
...
instance (Format din, Format dout) => Format (WrapF din dout) where
  ...
  showsf' (Wrap doutl din doutr) = showsFL $ doutl :. din :. End doutr
